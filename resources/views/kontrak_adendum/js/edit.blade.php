...
<script>
    $(document).ready(function() {
        ...
        let status_anggaran = "{{ $dataKontrak->jns_ang }}"
        let kodesubkegiatan = "{{ $kd_sub_kegiatan }}";
        ...
        $('#simpan').on('click', function() {
            ...
            let idKontrak = "{{ $dataKontrak->idkontrak }}"
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('kontrak_adendum.update') }}",
                        type: "POST",
                        dataType: 'json',
                        data: {
                            "_token": "{{ csrf_token() }}",
    ...
    function loadEdit(kd_sub_kegiatan, kd_rek6, kd_barang, sumber, header, sub_header) {
        $.ajax({
            ...
            data: {
                ...
                status_anggaran: "{{ $dataKontrak->jns_ang }}"
            },
            beforeSend: function() {
                $("#overlay").fadeIn(100);
            },
            success: function(data) {
                ...
                detailkontrak.bertingkat ? $('#bertingkat').prop('checked', true) : $('#bertingkat').prop(
                    'checked', false);
